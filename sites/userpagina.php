    ...
    <body>
        <div class="loginPanel">
            ...
            <div class="login-info">
                ...
                $_GET['identify'];
                $tweetOwner = User::loadUserById($conn, $_GET['identify']);
                ?>
            </div>
            <div class="login-info">   
        ...
        <div class="mainPanel">
            ...
            ?>
            <div class='tweetsByuser'>
                <h2 class="info">this is <?php echo $tweetOwner->getUsername(); ?>'s page</h2> 
                <form class="tweetForm" method="POST" action="">
                    <textarea class="message" type="text" max="140" name="text" 
                              placeholder="write a message to <?php echo $tweetOwner->getUsername(); ?>"></textarea>
                    <br>
                    <input class="messageButton" value="" type=submit name="send" />
                </form>
                <?php
                if ($_SERVER['REQUEST_METHOD']=='POST' && !empty($_POST['text']) && isset($_POST['send'])){
                    $message = new message();
                    $message ->setSenderId($_SESSION['id']);
                    $message ->setReciverId($tweetOwner->getId());
                ...
                <table>
                    <?php
                    $Tweets = Tweet::loadAllTweetsByUserId($conn, $_GET['identify']);
